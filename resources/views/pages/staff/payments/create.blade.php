{{-- resources/views/pages/staff/payments/create.blade.php --}}
...
@section('title', 'Add Payment for ' . $student->first_name . ' ' . $student->last_name)

{{-- Main Page Content --}}
@section('content')
    <div class="container-fluid">
        <div class="row justify-content-center">
            <div class="col-md-10 col-lg-10"> {{-- Adjusted column for better focus, made wider on large screens --}}

                {{-- Payment Form Card --}}
                <div class="card card-success card-outline">
                    <div class="card-header">
                        <h3 class="card-title">Enter Payment Details for: <span class="fw-bold">{{ $student->first_name }} {{ $student->last_name }}</span></h3>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="{{ route('staff.students.payments.store', $student->id) }}" id="payment-form">
                            @csrf
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4 form-group mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" name="amount" id="amount" class="form-control @error('amount') is-invalid @enderror"
                                               value="{{ old('amount') }}" placeholder="0.00" required step="0.01" min="0.01">
                                    </div>
                                    @error('amount')
                                ...
                                {{-- Lunch Payment Checkbox --}}
                                <div class="col-md-4 form-group mb-3 align-self-center pt-md-3">
                                    <div class="form-check mt-2">
                                        <input class="form-check-input" type="checkbox" name="lunch_payment" id="lunch_payment" value="1" {{ old('lunch_payment') ? 'checked' : '' }}>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <textarea name="comments" id="comments" class="form-control @error('comments') is-invalid @enderror"
                                          rows="2" placeholder="Optional payment notes (e.g., check number, payment method)">{{ old('comments') }}</textarea>
                                @error('comments')
                            ...
                            @if($unpaidFees->isNotEmpty())
                                <div class="table-responsive mb-3">
                                    <table class="table table-sm table-bordered table-hover align-middle">
                                        ...
                                        <tr>
                                            ...
                                            <th>Invoice #</th>
                                            <th style="width: 220px;" class="text-center">Amount to Apply</th> {{-- Wider for input + button --}}
                                            <th class="text-end">Remaining on Fee</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        @foreach($unpaidFees as $fee)
                                            <tr class="fee-allocation-row" data-fee-id="{{ $fee->id }}">
                                                <td>
                                                    <a href="{{ route('staff.fees.show', $fee->id) }}" target="_blank">{{ $fee->title }}</a>
                                                    <small class="d-block text-muted">Due: {{ $fee->due_date ? $fee->due_date->format('Y-m-d') : 'N/A' }}</small>
                                                </td>
                                                <td class="text-end original-balance-cell" data-original-balance="{{ $fee->balance }}">
                                                    {{ number_format($fee->balance, 2) }}
                                                </td>
                                                <td>{{ $fee->invoice_no ?? 'N/A' }}</td>
                                                <td>
                                                    <div class="input-group input-group-sm">
                                                        <input type="number" name="allocations[{{ $fee->id }}]"
                                                               class="form-control form-control-sm allocation-input @error('allocations.'.$fee->id) is-invalid @enderror"
                                                               value="{{ old('allocations.'.$fee->id, 0) }}"
                                                               step="0.01" min="0" max="{{ $fee->balance }}"
                                                               placeholder="0.00" aria-label="Amount to apply for {{ $fee->title }}">
                                ...
                                <div class="row justify-content-end mb-3">
                                    <div class="col-md-5 col-lg-4">
                                        ...
                                        @error('allocations') {{-- General allocation error --}}
                            ...
                            <div class="text-center border-top pt-3 mt-4">
                                ...
                                <a href="{{ route('staff.students.payments.index', $student->id) }}" class="btn btn-lg btn-secondary ms-2">
