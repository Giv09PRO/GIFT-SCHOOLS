{{-- resources/views/pages/staff/payments/allocate_unlinked_form.blade.php --}}
...
@section('title', 'Allocate Unlinked Payments')
    ...
    <div class="container-fluid">
        <div class="row justify-content-center">
            <div class="col-md-10 col-lg-9"> {{-- Adjusted column width --}}
                ...
                @include('layouts.partials.alerts') {{-- For server-side flash messages --}}

                {{-- Card 1: Unlinked Payments List & Selection --}}
                <div class="card card-primary card-outline shadow-sm mb-4">
                    <div class="card-header">
                        <h3 class="card-title"><i class="fas fa-search-dollar mr-1"></i> 1. Find and Select Unlinked Payment</h3>
                    </div>
                    <div class="card-body">
                        {{-- Filters for Unlinked Payments --}}
                        <form method="GET" action="{{ route('staff.payments.allocate_unlinked.form') }}" class="mb-3 p-3 border rounded bg-light">
                            <div class="row">
                                <div class="col-md-4 form-group">
                                    <label for="search_term_unlinked">Search (ID, Comment, Student)</label>
                                    <input type="text" name="search_term_unlinked" id="search_term_unlinked" class="form-control form-control-sm" value="{{ request('search_term_unlinked') }}" placeholder="Payment ID, comment, student name/ID">
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="unlinked_date_from">Date From</label>
                                    <input type="date" name="unlinked_date_from" id="unlinked_date_from" class="form-control form-control-sm" value="{{ request('unlinked_date_from') }}">
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="unlinked_date_to">Date To</label>
                                    <input type="date" name="unlinked_date_to" id="unlinked_date_to" class="form-control form-control-sm" value="{{ request('unlinked_date_to') }}">
                                </div>
                                        ...
                                        <tr id="payment_row_{{ $payment->id }}">
                                            ...
                                            <td>
                                                @if($payment->student)
                                                    ...
                                                    <small class="d-block text-muted">ID: {{ $payment->student->prem_number ?? 'N/A' }}</small>
                                                @else
                                                    <span class="text-muted">N/A</span>
                                                @endif
                                            </td>
                                            <td>{{ Str::limit($payment->comments, 50) }}</td>
                                            <td class="text-center">
                                                <button type="button" class="btn btn-xs btn-success btn-select-payment"
                                                        data-payment-id="{{ $payment->id }}"
                                                        data-payment-amount="{{ $payment->amount }}"
                                                        data-payment-date="{{ $payment->payment_date ? \Carbon\Carbon::parse($payment->payment_date)->format('Y-m-d') : '' }}"
                                                        data-payment-comments="{{ e($payment->comments) }}"
                                                        data-original-student-info="{{ $payment->student ? e($payment->student->first_name . ' ' . $payment->student->last_name . ' (ID: ' . ($payment->student->prem_number ?? 'N/A') . ')') : 'None' }}">
                ...
                <form method="POST" action="{{ route('staff.payments.allocate_unlinked.process') }}" id="allocate_unlinked_payment_form">
                    @csrf
                    <input type="hidden" name="payment_id" id="selected_payment_id_hidden" value="{{ old('payment_id') }}">

                    {{-- Card 2: Target Student Selection (Initially hidden) --}}
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <select id="target_student_search_select" name="target_student_id" class="form-control">
                                    ...
                                    @if(old('target_student_id'))
                                        {{-- This requires fetching the student's name if validation fails.
                                             For simplicity, this is often handled by re-querying in controller or with JS.
                                             Or, if you pass $targetStudent back on validation error:
                                        @if(isset($targetStudent))
                                        <option value="{{ $targetStudent->id }}" selected="selected">
                                            {{ $targetStudent->last_name }}, {{ $targetStudent->first_name }} (ID: {{ $targetStudent->prem_number ?? 'N/A' }})
                                        </option>
                                        @endif
                                        --}}
                                    @endif
                                ...
                                @error('target_student_id')
                                    <span class="d-block invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                @enderror
                            </div>
                        </div>
                    </div>

                    {{-- Card 3: Fee Allocation (Initially hidden) --}}
                    <div id="fee_allocation_card" class="card card-success card-outline shadow-sm mb-4 d-none">
                        ...
                        <div class="card-body">
                            <div id="student_fees_container">
                                {{-- Student's outstanding fees will be loaded here by JavaScript --}}
                                ...
                                <div id="no_outstanding_fees_message" class="alert alert-info d-none">
                                    This student has no outstanding fees for the current school year ({{ $syear }}).
                                    The payment will be linked to the student but remain unallocated.
                                </div>
                                @error('allocations') {{-- General error for allocations array --}}
                            ...
                            <div class="form-group mt-3">
                                ...
                                <textarea name="allocation_comment" id="allocation_comment" class="form-control @error('allocation_comment') is-invalid @enderror"
                                          rows="3" placeholder="Optional: Add comments about this specific allocation/linking process.">{{ old('allocation_comment') }}</textarea>
                                @error('allocation_comment')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#target_student_search_select').select2({
                ...
                ajax: {
                    url: '{{ route("staff.students.search_json") }}', // Ensure this route exists and returns students
                    dataType: 'json',
                    delay: 250,
                    data: function (params) {
                        return { search_term: params.term, page: params.page || 1, syear: '{{ $syear }}' };
                    },
            ...
            function fetchStudentFees(studentId) {
                // IMPORTANT: Create this route: GET /staff/students/{student}/fees-json?syear={{ $syear }}
                // It should return JSON like: { fees: [ {id, title, amount, total_paid, balance, due_date}, ... ] }
                // Ensure Fee model has 'balance' accessor.
                const feesUrl = `{{ url('staff/students') }}/${studentId}/fees-json?syear={{ $syear }}`;
            ...
            @if(old('payment_id') && old('target_student_id'))
                ...
                const oldPaymentId = {{ old('payment_id') }};
                ...
                if (oldPaymentButton.length) {
                    ...
                    // Example:
                    /*
                    @if(session('target_student_for_repopulation'))
                        var studentData = @json(session('target_student_for_repopulation'));
                   ...
                   // If allocations old data exists, try to repopulate
                    @if(is_array(old('allocations')))
                        setTimeout(function() { // Wait for fees to potentially load
                            @foreach(old('allocations') as $fee_id => $amount)
                                $(`input.allocation-input[data-fee-id="{{ $fee_id }}"]`).val({{ $amount }});
                            @endforeach
                            updateAllocationSummary();
            ...
            @if(session('flash_success'))
                Swal.fire('Success!', "{{ session('flash_success') }}", 'success');
            @endif
            @if(session('flash_danger'))
                Swal.fire('Error!', "{{ session('flash_danger') }}", 'error');
            @endif
            @if(session('flash_warning'))
                Swal.fire('Warning!', "{{ session('flash_warning') }}", 'warning');
            @endif
            @if(session('flash_info'))
                Swal.fire('Info!', "{{ session('flash_info') }}", 'info');
            @endif
